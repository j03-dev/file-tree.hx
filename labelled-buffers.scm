...
(require (only-in "helix/keymaps.scm" *reverse-buffer-map-insert*))
...
(define (open-or-create-labelled-buffer! #:label label
                                         #:language-type (language-type #f)
                                         #:side (side 'none))
  ...
  (if maybe-doc-id
      ...
      (begin
        (make-new-labelled-buffer! #:label label #:language-type language-type #:side side)
